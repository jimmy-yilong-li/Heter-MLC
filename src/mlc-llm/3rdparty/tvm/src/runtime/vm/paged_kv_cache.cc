...
class PagedAttentionKVCacheObj : public AttentionKVCacheObj {
 ...
 public:
  ...
  void DebugSetKV(int64_t seq_id, int64_t start_pos, Tensor k_data, Tensor v_data) final {
    // Inverse of DebugGetKV: scatter externally-computed K/V into the paged cache for `seq_id`.
    // Reuses the standard `f_transpose_append_mha` append kernel (a pure scatter into pages),
    // applied per layer over the same physical position map DebugGetKV builds. The sequence must
    // already be grown to hold [start_pos, start_pos + length) (e.g. via BeginForward /
    // DisaggPrepareRecv, both of which allocate pages through the BeginForward reserve path).
    TVM_FFI_ICHECK(f_transpose_append_mha_.defined())
        << "PageAttentionKVCache requires `f_transpose_append_mha` to set K/V data.";

    // k/v_data: (num_layers, length, num_kv_heads, qk_head_dim)
    static constexpr const char* error_msg =
        "DebugSetKV expects the k_data in layout (num_layers, length, num_kv_heads, qk_head_dim).";
    std::vector<Tensor*> vec_kv_data = {&k_data, &v_data};
    for (const Tensor* data_ptr : vec_kv_data) {
      TVM_FFI_ICHECK_EQ((*data_ptr)->ndim, 4) << error_msg;
      TVM_FFI_ICHECK_EQ((*data_ptr)->shape[0], num_layers_)
    ...
    TVM_FFI_ICHECK_EQ(k_data->shape[1], v_data->shape[1])
        << "DebugSetKV expects K and V to have the same length.";
    TVM_FFI_ICHECK(k_data->dtype.code == v_data->dtype.code &&
                   k_data->dtype.bits == v_data->dtype.bits &&
                   k_data->dtype.lanes == v_data->dtype.lanes)
        << "DebugSetKV expects K and V to share the same dtype (the per-layer byte stride is "
           "computed from K's dtype and reused for V's view).";
    TVM_FFI_ICHECK(k_data.DataType() == pages_[0].DataType())
        << "DebugSetKV expects K/V dtype to match the cache dtype. Got K=" << k_data.DataType()
        << ", cache=" << pages_[0].DataType() << ".";
    ...
    TVM_FFI_ICHECK_LE(start_pos + length, seq.seq_length)
        << "DebugSetKV does not accept out-of-range [start_pos, start_pos + length). The sequence "
           "must already be grown (e.g. via DisaggPrepareRecv/BeginForward) to hold these tokens.";
    ...
    //
    // We materialize each layer's slice into a freshly allocated offset-0 tensor instead of
    // passing k_data.CreateView(..., layer_id * stride) directly to the kernel. The append
    // kernel is declared as T.match_buffer over a 3-D `(ntoken, heads, dim)` buffer, and a view
    // carved out of a 4-D tensor with a non-zero byte_offset reads out of bounds for layer_id
    // >= 1 (uncaught tvm::ffi::Error, reproduced on CPU and Metal). Materializing each layer
    // separately means the kernel only ever sees a byte_offset of 0.
    //
    // The slice->slice copy uses Tensor::CopyFrom (which dispatches through CopyFromTo), so it
    // is device-agnostic and works on both CPU and Metal without dereferencing device memory
    // on the host.
    const int64_t bytes_per_elem = (k_data->dtype.bits * k_data->dtype.lanes + 7) / 8;
    const int64_t layer_stride_bytes = length * num_kv_heads_ * qk_head_dim_ * bytes_per_elem;
    for (int64_t layer_id = 0; layer_id < num_layers_; ++layer_id) {
      TVM_FFI_ICHECK(attn_kinds_[layer_id] == AttnKind::kMHA)
          << "Only MHA is supported for DebugSetKV";
      Tensor k_layer = Tensor::Empty({length, num_kv_heads_, qk_head_dim_}, k_data->dtype, device_);
      Tensor v_layer = Tensor::Empty({length, num_kv_heads_, qk_head_dim_}, v_data->dtype, device_);
      // CreateView produces a byte-offset view of the layer slice; CopyFrom copies it into the
      // fresh offset-0 tensor that the kernel will consume.
      Tensor k_view = k_data.CreateView({length, num_kv_heads_, qk_head_dim_}, k_data->dtype,
                                        layer_id * layer_stride_bytes);
      Tensor v_view = v_data.CreateView({length, num_kv_heads_, qk_head_dim_}, v_data->dtype,
                                        layer_id * layer_stride_bytes);
      k_layer.CopyFrom(k_view);
      v_layer.CopyFrom(v_view);
      f_transpose_append_mha_.value()(pages_[layer_id], k_layer, v_layer, position_map_device);
    }
  }
