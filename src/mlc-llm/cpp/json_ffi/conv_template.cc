...
static std::string ContentPartValueToString(const tvm::ffi::json::Value& value) {
  if (auto str = value.try_cast<std::string>()) {
    return str.value();
  }
  if (auto str = value.try_cast<tvm::ffi::String>()) {
    return str.value();
  }
  return tvm::ffi::json::Stringify(value);
}
    ...
    if (message_arr[1].try_cast<std::string>().has_value()) {
      ...
      for (const auto& item : content_arr) {
        ...
        for (const auto& [key, value] : item.cast<tvm::ffi::json::Object>()) {
          item_map[key.cast<tvm::ffi::String>()] = ContentPartValueToString(value);
        }
